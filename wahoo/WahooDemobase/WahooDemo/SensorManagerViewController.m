...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self)
    {
        ...
        discoveredSensors = [NSMutableArray arrayWithCapacity:10];
        isSearching = FALSE;
    ...
    if ( tableView == pairedTable )
    {
        if (editingStyle == UITableViewCellEditingStyleDelete)
        {
            ...
            if ( [settings removeDeviceParams:dp forSensorType:sensorType] )
            {
                ...
                deviceParams = [settings deviceParamsForSensorType:sensorType];
...
- (void)configForSensorType:(WFSensorType_t)eSensorType onNetworks:(USHORT)usNetworks
{
	...
	deviceParams = [[WFHardwareConnector sharedConnector].settings deviceParamsForSensorType:sensorType];
    isSearching = FALSE;
